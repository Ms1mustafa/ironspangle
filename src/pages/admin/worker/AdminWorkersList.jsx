...
export default function AdminWorkersList() {
  ...
  const calculateOverallTotalRecevied = () => {
    return workers
      .reduce((total, worker) => {
        const { salary, insurance2, active_days } = worker;
        return total + Number(salary * active_days) - Number(insurance2);
      }, 0)
      .toLocaleString();
  };
  ...
  const footerGroup = workers.length > 0 && (
    <ColumnGroup>
      <Row>
        ...
        />
        <Column
          footer={calculateOverallTotalRecevied}
          footerStyle={{
            // textAlign: "center",
            backgroundColor: "yellow",
            color: "#000",
      ...
      <div className="card">
        ...
        >
          ...
          <Column
            header="Received"
            body={(rowData) =>
              rowData?.salary && rowData?.active_days && rowData?.insurance2
                ? (
                    Number(rowData.salary * rowData.active_days) -
